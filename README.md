...
Tools: `bitrix_db_connections` (list active connections, passwords redacted), `bitrix_db_schema` (tables and columns), `bitrix_db_query` (read-only SQL). Writes are opt-in: `bitrix_db_execute` (INSERT/UPDATE/DELETE) when `BITRIX_MCP_DB_ALLOW_WRITE=1`. `bitrix_db_query` rejects write/lock/file statements and side-effecting functions, runs inside a read-only transaction with a server-side timeout, and caps rows and bytes; for a hard guarantee point it at a `SELECT`-only account via `BITRIX_MCP_DB_READONLY_USER` / `BITRIX_MCP_DB_READONLY_PASSWORD`. Passwords are redacted and credential files (`.settings.php`, `dbconn.php`, `.env`, …) are never returned by file-reading tools, but table data can still contain secrets — see [Security](./docs/security.md). Intended for local development only. `init` prompts whether to enable DB access (default yes) and writes (default no); use `--no-db` and `--db-allow-write` to control it non-interactively.
